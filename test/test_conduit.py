...
class TestConduit(object):
    ...
    def test_accept_cookie(self):
        cookie_bar = WebDriverWait(self.browser, 3).until(
            EC.presence_of_element_located((By.XPATH, '//div[@class = "cookie__bar__buttons"]')))
        assert cookie_bar.is_displayed()

        cookie_bar_buttons_list_before_click = self.browser.find_elements_by_xpath('//div[@class="cookie__bar__buttons"]')
        assert len(cookie_bar_buttons_list_before_click) != 0

        cookie_btn_accept = WebDriverWait(self.browser, 3).until(EC.presence_of_element_located(
            (By.XPATH, '//button[@class ="cookie__bar__buttons__button cookie__bar__buttons__button--accept"]')))
        cookie_btn_accept.click()

        time.sleep(3)

        cookie_bar_buttons_list_after_click = self.browser.find_elements_by_xpath('//div[@class="cookie__bar__buttons"]')
        assert len(cookie_bar_buttons_list_after_click) == 0
    ...
    def test_registration(self):
        ...
        assert invalid_email_msg.text == messages["invalid_email"]
        assert reg_failure_alert.text == messages["reg_failure"]
    ...
    def test_login(self):
        ...
        logged_in_user_name = WebDriverWait(self.browser, 3).until(
           EC.presence_of_element_located((By.XPATH, '//a[@class="nav-link" and @href="#/@Teszt/"]')))
    ...
    def test_logout(self):
        ...
        logout_btn = WebDriverWait(self.browser, 3).until(
            EC.presence_of_element_located((By.XPATH, '//a[@active-class="active"]')))

        logout_btn.click()
        time.sleep(3)

        sign_in_btn = WebDriverWait(self.browser, 3).until(
    ...
    def test_import_data_from_file(self):
        ...
        articles_list_initial = WebDriverWait(self.browser, 3).until(
            EC.presence_of_all_elements_located((By.XPATH, '//a[@class="preview-link"] ')))

        all_articles_counter = len(articles_list_initial)
        ...
        with open("test/articles.csv", "r") as csvfile:
            ...
            for row in csvreader:
                ...
                publish_btn = WebDriverWait(self.browser, 4).until(
        ...
        assert all_articles_counter - my_articles_counter == len(articles_list_initial)
    ...
    def test_create_new_element(self):
        ...
        posted_comments = WebDriverWait(self.browser, 3).until(
    ...
    def test_update_profile_picture(self):
        ...
        settings = WebDriverWait(self.browser, 3).until(
        ...
        update_btn = WebDriverWait(self.browser, 3).until(
            EC.presence_of_element_located((By.XPATH, '//button[@class="btn btn-lg btn-primary pull-xs-right"]')))
        update_btn.click()

        confirm_btn = WebDriverWait(self.browser, 3).until(
        ...
        profile_pic = WebDriverWait(self.browser, 4).until(
    ...
    def test_list_elements(self):
        ...
        ipsum_tagged_articles_main = WebDriverWait(self.browser, 3).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, '//a[@href="#/tag/ipsum" and @class="tag-pill tag-default"]')))

        ipsum_tag = self.browser.find_element_by_xpath('//a[@href = "#/tag/ipsum"]')
        ipsum_tag.click()

        ipsum_tag_articles_filtered = WebDriverWait(self.browser, 3).until(
    ...
    def test_pagination(self):
        ...
        pagination_buttons = WebDriverWait(self.browser, 3).until(
