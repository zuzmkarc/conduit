...
def registration(browser, username, email, password):
    sign_up_btn = WebDriverWait(browser, 4).until(
        EC.presence_of_element_located((By.XPATH, '//a[@href="#/register"]')))
    sign_up_btn.click()

    username_field = WebDriverWait(browser, 4).until(
    ...
    confirm_btn = WebDriverWait(browser, 4).until(
        EC.presence_of_element_located((By.XPATH, '//button[@class="swal-button swal-button--confirm"]')))
    confirm_btn.click()

    logout_btn = WebDriverWait(browser, 4).until(
        EC.presence_of_element_located((By.XPATH, '//a[@active-class="active"]')))
    logout_btn.click()


def login(browser, email, password):
    sign_in_btn = WebDriverWait(browser, 4).until(EC.presence_of_element_located((By.XPATH, '//a[@href="#/login"]')))
    sign_in_btn.click()
    time.sleep(3)

    email_field = WebDriverWait(browser, 4).until(EC.presence_of_element_located((By.XPATH, '//input[@type="text"]')))
    email_field.send_keys(email)
...
def click_logged_in_user_name(browser):
    logged_in_user_name = WebDriverWait(browser, 4).until(
...
def find_and_clear_element(browser, element_xpath):
    web_element = WebDriverWait(browser, 4).until(EC.presence_of_element_located((By.XPATH, element_xpath)))
    web_element.clear()
    return web_element
